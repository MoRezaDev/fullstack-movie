...
export default function SwiperWrapper({ data }: { data: any }) {
  ...
  return (
    <div className="w-full">
      <div className="w-full h-[400px] sm:h-[480px] mb-4 relative">
        <div
          dir="rtl"
          className="absolute top-0 right-0 h-full w-full md:w-[50%] px-6 py-10 md:px-16 md:py-28 z-2"
        >
          <PosterCard data={data[dataIndex]} />
        </div>
        <div className="w-full relative h-full flex items-center">
          <div dir="rtl" className="absolute inset-0 bg-black overflow-hidden">
            <div className="w-full md:w-[55%] relative backdroptest h-full">
              <img src={bgUrl} className="size-full object-cover" />
              <div></div>
            </div>
          </div>
          <div className="max-w-[640px] hidden md:block ml-8  relative  w-[95%] overflow-hidden mt-4 self-end mb-4">
            <Swiper
              loop={true}
      ...
      <SwipperWrapperMobile data={data} setBgUrl={setBgUrl} />
    </div>
  );
}
